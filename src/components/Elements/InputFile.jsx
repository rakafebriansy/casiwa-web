...
const InputFile = (props) => {
    const {name, message, id = 'dropzone-file', dropzone = false} = props;
    ...
    if(dropzone) {
        return (
            <div onDragOver={handleDragOver} onDrop={handleDrop} className="flex items-center justify-center w-full input-file">
                <label htmlFor={id} className={`flex flex-col items-center justify-center w-full h-40 border-2 ${files && files.length > 0 ? 'border-primary' : 'border-gray-300'} border-dashed rounded-lg cursor-pointer text-gray-500 hover:text-primary hover:border-primary ${files && files.length > 0 ? 'bg-gray-100' : 'bg-gray-50'} hover:bg-gray-100`}>
                    ...
                    <input name={name} id={id} type="file" className="hidden" onChange={e => {setFiles(e.target.files);}} ref={inputRef}/>
                </label>
            </div> 
        );
    }
