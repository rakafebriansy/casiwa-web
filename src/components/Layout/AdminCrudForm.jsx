                    ...
                    <div className="p-1.5 min-w-full inline-block align-middle">
                        <div className="overflow-y-scroll max-h-40">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        <th scope="col" className="px-3 py-3 text-start text-xs font-montserratMedium text-gray-500 uppercase">{columnName}</th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {list.map(item => {
                                        return (
                                            <tr key={item.id}>
                                                <td className="px-3 py-4 whitespace-nowrap text-sm font-montserratMedium text-gray-800">{item.name}</td>
                                                <td className="px-3 py-4 whitespace-nowrap text-end text-sm font-montserratMedium flex gap-4 justify-start">
                                                    <button type="button" onClick={() => openEditModal(item.id, item.name)} className="inline-flex select-none items-center gap-x-2 text-sm font-montserratSemiBold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 focus:outline-none focus:text-blue-800 disabled:opacity-50 disabled:pointer-events-none">Edit</button>
                                                    <button type="button" onClick={() => openDeleteModal(item.id, item.name)} className="inline-flex select-none items-center gap-x-2 text-sm font-montserratSemiBold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 focus:outline-none focus:text-blue-800 disabled:opacity-50 disabled:pointer-events-none">Delete</button>
                                                </td>
                                            </tr>
                                        )
                                    })}
